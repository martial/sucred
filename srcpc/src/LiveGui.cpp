...
void LiveGui::populate () {
   ...
   // addTextArea("PLAYER", "PLAYER", OFX_UI_FONT_MEDIUM);
	...
	palyndromOverBtn = addToggle("LOOP PALYNDROME OVER",  false, OFX_UI_FONT_SMALL);
...
void LiveGui::onGuiEvent(ofxUIEventArgs & e) {
    ...
    if (name == "STROB / DMX") {
		...
		Globals::get()->sceneManager->getScene(0)->setStrobDmx(e.getToggle()->getValue());
